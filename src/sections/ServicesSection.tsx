...
const ServiceCard = ({
  ...
  return (
    <>
      <motion.div
        className="absolute left-0 right-0 mx-auto w-[95%] sm:w-[85%] lg:w-[65%] xl:w-[60%] rounded-[30px] sm:rounded-[40px] md:rounded-[50px]
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-3 sm:gap-4 flex-1 min-h-0">
          {/* Left column - 40% (Buttons) */}
          <div className="w-full md:w-[40%] flex flex-row md:flex-col gap-2 sm:gap-3 md:gap-4 flex-shrink-0 md:flex-shrink">
            
            {/* Top Button Block (Let's Talk) */}
            <button 
              onClick={() => setIsModalOpen(true)}
              className="relative w-full flex-1 md:flex-[2] min-h-0 rounded-[20px] sm:rounded-[24px] md:rounded-[40px] bg-[#0C0C0C] flex flex-row items-center justify-center gap-2 sm:gap-3 md:gap-4 group transition-all duration-300 hover:bg-[#1A1A1A] border border-[#0C0C0C]/10 overflow-hidden p-3 sm:p-4"
            >
              <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              <span className="text-[#D7E2EA] font-black uppercase tracking-widest whitespace-nowrap text-sm sm:text-lg md:text-2xl lg:text-3xl">
                Let's Talk
              </span>
              <MessageCircle className="w-5 h-5 sm:w-7 sm:h-7 md:w-9 md:h-9 text-[#D7E2EA] group-hover:scale-110 transition-transform duration-300" strokeWidth={2} />
            </button>

            {/* Bottom Button Block (View Work) */}
            <a 
              href={`#all-projects?category=${service.slug}`}
              className="relative w-full flex-1 md:flex-[3] min-h-0 rounded-[20px] sm:rounded-[24px] md:rounded-[40px] bg-[#0C0C0C] flex flex-row items-center justify-center gap-2 sm:gap-3 md:gap-4 group transition-all duration-300 hover:bg-[#1A1A1A] border border-[#0C0C0C]/10 overflow-hidden p-3 sm:p-4"
            >
              <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              <span className="text-[#D7E2EA] font-black uppercase tracking-widest whitespace-nowrap text-sm sm:text-lg md:text-2xl lg:text-3xl">
                View Work
              </span>
              <ArrowRight className="w-5 h-5 sm:w-7 sm:h-7 md:w-9 md:h-9 text-[#D7E2EA] group-hover:translate-x-2 transition-transform duration-300" strokeWidth={2} />
            </a>

          </div>

          {/* Right column - 60% (Image) */}
          <div className="w-full md:w-[60%] flex-1 min-h-0">
            <img
              src={service.col2Image}
              alt={`${service.name} main`}
              className="w-full h-full object-cover rounded-[20px] sm:rounded-[24px] md:rounded-[40px]"
            />
          </div>
        </div>
      </motion.div>
